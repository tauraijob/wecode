<template>
  <section class="mx-auto max-w-7xl px-3 sm:px-4 py-8">
    ...
    <div class="mb-6 sm:mb-8">
      ...
      <p class="mt-1 sm:mt-2 text-sm sm:text-base text-navy-300">Manage training and service requests</p>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-navy-800/50 border-b border-navy-700/50">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-navy-200">User</th>
          ...
          <tbody class="divide-y divide-navy-700/50">
            <tr v-for="r in rows" :key="r.id" class="hover:bg-navy-800/30 transition-colors">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ r.user?.name || r.user?.email }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-navy-300">{{ r.category }}</td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                <button
                  ...
                  @click="approve(r.id)"
                  class="rounded-lg border border-green-600 bg-green-500/10 px-4 py-1.5 text-sm font-medium text-green-400 hover:bg-green-500/20 transition-all"
                >
                  Approve
                </button>
              </td>
    ...
    <div v-if="rows.length === 0 && !loading" class="mt-8 rounded-2xl border border-navy-700/50 bg-gradient-to-br from-navy-900/80 to-navy-800/40 p-12 text-center">
      ...
      <p class="text-navy-300">{{ q || status ? 'Try adjusting your filters' : 'No requests submitted yet' }}</p>
    </div>
  </section>
</template>
