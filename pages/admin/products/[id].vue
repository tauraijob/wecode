<template>
  <section class="mx-auto max-w-4xl px-3 sm:px-4 py-8">
    ...
    <div v-else-if="!product" class="text-center py-20">
      <p class="text-navy-400">Product not found</p>
      <NuxtLink to="/admin/products" class="mt-4 inline-block text-purple-400 hover:text-purple-300">
        Back to Products
      </NuxtLink>
    </div>
...
const { data: product, refresh, pending: loading } = await useFetch(`/api/admin/products/${productId}`)
...
watch(product, (newProduct) => {
  if (newProduct) {
    ...
    form.tags = newProduct.tags || ''
    form.images = (newProduct.images as any) || []
    form.featured = newProduct.featured
    form.weight = newProduct.weight ? Number(newProduct.weight) : null
